...
function studentLogin() {
  ...
  if (!validSid(sid)) {
    showErr(err, 'Student ID must be in format 25-XXXX-XXX');
    return;
  }
  if (!pass) {
    showErr(err, 'Password is required');
    return;
  }
  ...
  if (!stu || stu.password !== pass) {
    showErr(err, 'Invalid Student ID or password');
    return;
  }
...
function studentRegister() {
  ...
  for (var i = 0; i < DB.students.length; i++) {
    if (DB.students[i].sid === sid) {
      showErr(err, 'This Student ID is already registered');
      return;
    }
  }
...
function switchStuTab(name, btn) {
  document.querySelectorAll('#screen-student .tab-content').forEach(function (el) {
    el.classList.remove('active');
  });
  document.querySelectorAll('#screen-student .tab-btn').forEach(function (el) {
    el.classList.remove('active');
  });
...
function renderStuBrowse() {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    html += '<div class="book-top"><span class="cat-pill" style="' + catStyle(b.category) + '">' + esc(b.category) + '</span>';
...
function requestBorrow(bookId) {
  ...
  if (book.availableCopies < 1) {
    toast('Unavailable', 'No copies left of this book.', true);
    return;
  }
  ...
  for (var i = 0; i < DB.records.length; i++) {
    ...
    if (r.studentId === stu.id && r.bookId === bookId && (r.status === 'pending' || r.status === 'borrowed' || r.status === 'overdue')) {
      toast('Already requested', 'You already have an active request or loan for this book.', true);
      return;
    }
  }
  ...
  if (active >= 5) {
    toast('Limit reached', 'You can have at most 5 active requests/loans.', true);
    return;
  }
...
function renderStuMyBooks() {
  ...
  if (!mine.length) {
    wrap.innerHTML = emptyBox('open', 'No borrow records', 'Request a book from the Browse tab.');
    return;
  }
...
function handlePhotoChange(input) {
  ...
  if (file.size > 800 * 1024) {
    toast('Photo too large', 'Please choose an image under 800 KB.', true);
    return;
  }
